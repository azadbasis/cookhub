...
def search(request):
    keyword = request.GET.get('keyword', '')  # Get the 'keyword' parameter from the request, default to an empty string if not provided

    if keyword:
        recipes = Recipe.objects.order_by('name').filter(Q(name__icontains=keyword) | Q(description__icontains=keyword))
        recipe_count = recipes.count()
    else:
        # Handle the case when the keyword is empty (e.g., no keyword provided)
        recipes = []  # You can set recipes to an empty list or handle it as needed
        recipe_count = 0
